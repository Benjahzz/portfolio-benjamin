...
const Bubble: React.FC<BubbleProps> = ({ icon, className }) => {
  ...
  return (
    <span ref={bubbleRef} className={cn(`bubble p-4 fixed rounded-full text-2xl z-10 md:cursor-pointer animate-floating border dark:border-none  ${isTriggered ? '--active cursor-none' : ''}`, className, bubbleIcon[icon].backgroundColor)} onClick={onClick}>
